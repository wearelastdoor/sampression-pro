...
/**
 * The template for default post format.
 *
...
<article id="post-<?php the_ID(); ?>" class="post <?php echo sampression_column_class(); ?> <?php echo sampression_cat_slug(); ?> <?php if ( get_the_ID() == $sticky && is_home() ) { echo 'sticky corner-stamp'; } else { echo 'item'; } ?> ">
    ...
    <?php if( ( $options['show_meta_author'] == 'yes' ) || ( $options['show_meta_date'] == 'yes' ) ) { ?>
        <div class="meta clearfix">
            ...
            <?php if( $options['show_meta_date'] == 'yes' ) { //show/hide metas date on single post from theme option (post meta settings) ?>
                <time class="col" datetime="<?php echo get_the_time( 'Y-m-d' ) ?>"> <span class="icon-Calendar-4"></span> <a href="<?php echo get_month_link( get_the_time( 'Y' ), get_the_time( 'm' ) ); ?>" title="<?php echo get_the_time(); ?>" rel="bookmark"> <?php echo get_the_date(); ?></a></time>
            <?php } //endif ?>
    ...
    ?>
    <?php if( $options['show_meta_categories'] == 'yes' && get_post_type() == 'post') { //show/hide meta categories on single post from theme option (post meta settings)?>
        <div class="meta">
